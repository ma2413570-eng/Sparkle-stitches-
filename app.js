...
.product-detail{display:grid;gap:16px}.product-detail-img{width:100%;max-height:340px;object-fit:cover;border-radius:18px}.size-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.size-option{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:11px 12px;border:1px solid rgba(255,255,255,.10);border-radius:14px;cursor:pointer}.size-option.selected{border-color:var(--gold);background:rgba(212,175,55,.09)}.size-option input{accent-color:var(--gold)}.size-price{color:var(--gold);font-weight:700}.size-admin-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.size-admin-grid label{display:grid;gap:5px;font-size:12px;color:#aaa}.touch-ripple{position:absolute;width:18px;height:18px;border-radius:999px;pointer-events:none;background:radial-gradient(circle,rgba(255,255,255,.42) 0%,rgba(212,175,55,.20) 45%,rgba(212,175,55,0) 72%);transform:translate(-50%,-50%) scale(1);animation:liquidTouch .65s ease-out forwards;z-index:5}.btn{position:relative;overflow:hidden;-webkit-tap-highlight-color:transparent}.btn:focus{outline:none}.btn:focus-visible{outline:1px solid rgba(212,175,55,.55);outline-offset:2px}@keyframes liquidTouch{to{transform:translate(-50%,-50%) scale(9);opacity:0}}.intro-product{position:fixed;left:50%;top:12%;transform:translate(-50%,-35px) scale(.92);width:70px;height:70px;border-radius:18px;object-fit:cover;box-shadow:0 18px 50px rgba(0,0,0,.25);opacity:0;pointer-events:none;z-index:9999;animation:miniProductDrop .9s cubic-bezier(.2,.8,.2,1) forwards}.intro-product::after{content:''}@keyframes miniProductDrop{0%{opacity:0;transform:translate(-50%,-45px) scale(.88)}25%{opacity:.9}72%{opacity:.8;transform:translate(-50%,18px) scale(1)}100%{opacity:0;transform:translate(-50%,45px) scale(.96)}}

</style>`;drawProducts()}

function showIntroProduct(){
  if(document.body.dataset.sparkleIntro)return;
  document.body.dataset.sparkleIntro='1';
  const img=document.createElement('img');img.className='intro-product';img.src=(products[0]?.image_url||fallbackImg);img.alt='';
  document.body.appendChild(img);setTimeout(()=>img.remove(),1000);
}

function drawProducts(){
  let q=($('#search')?.value||'').toLowerCase();
  let list=products.filter(p=>(p.name+' '+p.category+' '+p.description).toLowerCase().includes(q));
  let el=$('#products');if(!el)return;
  el.innerHTML=list.length?list.map(p=>`<article class="card glass" onclick="openProduct('${p.id}')"><img class="card-img" src="${esc(p.image_url||fallbackImg)}" onerror="this.src='logo.jpg'"><div class="card-body"><div class="tag">${esc(p.category)}</div><h3>${esc(p.name)}</h3><p class="muted">${esc(p.description)}</p><div class="row"><div class="price">${money(p.price)}</div><button class="btn gold" onclick="event.stopPropagation();openProduct('${p.id}')">View</button></div></div></article>`).join(''):`<div class="empty" style="grid-column:1/-1">No designs yet. Add your first design from the admin panel.</div>`;
}
...
function getSizePrices(p){
  let raw=p?.size_prices;
  if(raw&&typeof raw==='object')return raw;
  try{return raw?JSON.parse(raw):{}}catch(e){return {}}
}
function availableSizes(p){
  const prices=getSizePrices(p);
  const keys=Object.keys(prices).filter(k=>Number(prices[k])>=0);
  return keys.length?keys:['XS','S','M','L','XL','XXL'];
}
function priceForSize(p,size){
  const prices=getSizePrices(p);
  return Number(prices[size] ?? p.price ?? 0);
}
function openProduct(id){
  const p=products.find(x=>x.id===id);if(!p)return;
  const sizes=availableSizes(p),prices=getSizePrices(p),defaultSize=sizes[0];
  modal(`<div class="modal-head"><div><div class="eyebrow">${esc(p.category)}</div><h3>${esc(p.name)}</h3></div><button class="btn" onclick="closeModal()">×</button></div><div class="product-detail"><img class="product-detail-img" src="${esc(p.image_url||fallbackImg)}" onerror="this.src='logo.jpg'"><p class="muted">${esc(p.description)}</p><div><div class="eyebrow">Choose size</div><div class="size-grid">${sizes.map((s,i)=>`<label class="size-option ${i===0?'selected':''}" onclick="selectSizeOption(this)"><span><input type="radio" name="productSize" value="${esc(s)}" ${i===0?'checked':''}> ${esc(s)}</span><span class="size-price">${money(priceForSize(p,s))}</span></label>`).join('')}</div></div><div class="row"><strong id="selectedProductPrice">${money(priceForSize(p,defaultSize))}</strong><button class="btn gold" onclick="addSelectedToCart('${p.id}')">Add to bag</button></div></div>`);
}
function selectSizeOption(el){
  document.querySelectorAll('.size-option').forEach(x=>x.classList.remove('selected'));el.classList.add('selected');
}
function addSelectedToCart(id){
  const p=products.find(x=>x.id===id),size=document.querySelector('input[name="productSize"]:checked')?.value;
  if(!p||!size)return;
  addToCart(id,size);
  closeModal();
}
function addToCart(id,size){
  ...
  size=size||availableSizes(p)[0];
  let unitPrice=priceForSize(p,size);
  let x=cart.find(x=>x.id===id&&x.size===size);
  if(x)x.qty++;else cart.push({id,qty:1,size,unitPrice});
  saveCart();toast(`${p.name} • ${size} added to your bag ✨`);
}
...
function openCart(){
  let rows=cart.map(x=>{
    let p=products.find(p=>p.id===x.id);if(!p)return '';
    const unit=Number(x.unitPrice??priceForSize(p,x.size));
    return `<div class="cart-item"><img src="${esc(p.image_url||fallbackImg)}"><div><b>${esc(p.name)}</b><div class="muted">Size: ${esc(x.size||'Standard')} · ${money(unit)}</div></div><div class="qty"><button onclick="changeQty('${p.id}',-1,'${esc(x.size||'')}')">−</button><b>${x.qty}</b><button onclick="changeQty('${p.id}',1,'${esc(x.size||'')}')">+</button></div></div>`;
  }).join('');
  let total=cart.reduce((a,x)=>{let p=products.find(p=>p.id===x.id);return a+(p?(Number(x.unitPrice??priceForSize(p,x.size))*x.qty):0)},0);
  modal(`<div class="modal-head"><div><div class="eyebrow">Your selection</div><h3>Shopping bag</h3></div><button class="btn" onclick="closeModal()">×</button></div>${rows||'<div class="empty">Your bag is waiting for something beautiful.</div>'}${rows?`<div class="row" style="padding-top:18px"><b>Total</b><strong style="font-size:25px;color:var(--gold)">${money(total)}</strong></div><button class="btn gold" style="width:100%;margin-top:15px" onclick="checkout()">Continue to checkout</button>`:''}`);
}
function changeQty(id,d,size){let x=cart.find(x=>x.id===id&&x.size===(size||x.size));if(!x)return;x.qty+=d;if(x.qty<=0)cart=cart.filter(x=>!(x.id===id&&x.size===(size||x.size)));saveCart();openCart()}
...
async function openMyOrders(){
  ...
  const cards=(orders||[]).map(o=>{
    ...
    return `<article class="order-card glass">
      ...
      <div class="order-items">${its.map(i=>`<div class="order-item"><span>${esc(i.product_name)}${i.size?` • Size ${esc(i.size)}`:''} × ${i.quantity}</span><b>${money(Number(i.unit_price)*Number(i.quantity))}</b></div>`).join('')||'<div class="muted">Order details unavailable.</div>'}</div>
...
async function verifyEmailOtp(email){let token=$('#otpEmail').value.trim();let {data,error}=await db.auth.verifyOtp({email,token,type:'email'});if(error)return toast(error.message);user=data.user;await refreshAdmin();try{await db.from('login_activity').insert({user_id:user.id,email:user.email,is_admin:isAdmin})}catch(e){console.warn('login activity',e)}closeModal();toast(isAdmin?'Admin login successful 👑':'Welcome to Sparkle Stitches ✨');render()}
...
async function adminLoginActivity(){
  ...
  if(error)return toast(error.message);
  $('#adminContent').innerHTML=`<div class="eyebrow">Security</div><h3>Login activity</h3><p class="muted">Admins can see email, account type and login time. OTPs and passwords are never stored.</p><div class="table-wrap glass"><table class="table"><tr><th>Email</th><th>Type</th><th>Login time</th></tr>${(data||[]).map(x=>`<tr><td>${esc(x.email)}</td><td>${x.is_admin?'Admin':'Customer'}</td><td>${new Date(x.created_at).toLocaleString('en-IN')}</td></tr>`).join('')||'<tr><td colspan="3" class="empty">No login activity yet.</td></tr>'}</table></div>`;
}
...
async function editProduct(id){
  let p=id?products.find(x=>x.id===id):null,sp=getSizePrices(p||{});
  const sizes=['XS','S','M','L','XL','XXL'];
  modal(`<div class="modal-head"><div><div class="eyebrow">Catalog</div><h3>${p?'Edit design':'New design'}</h3></div><button class="btn" onclick="adminHome()">×</button></div><form class="form" onsubmit="saveProduct(event,'${p?.id||''}')"><input class="input" name="name" placeholder="Product name" value="${esc(p?.name||'')}" required><div class="two"><input class="input" name="price" type="number" min="0" step="1" placeholder="Default price" value="${p?.price||''}" required><input class="input" name="category" placeholder="Category" value="${esc(p?.category||'Blouse Ari Work')}" required></div><textarea class="textarea" name="description" rows="3" placeholder="Description">${esc(p?.description||'')}</textarea><label class="muted">Separate price for each size (leave blank to use default price)</label><div class="size-admin-grid">${sizes.map(s=>`<label>${s}<input class="input" type="number" min="0" step="1" name="size_${s}" value="${sp[s]??''}" placeholder="₹ price"></label>`).join('')}</div><input class="input" name="image_url" placeholder="Image URL (optional)" value="${esc(p?.image_url||'')}"><select class="select" name="active"><option value="true" ${p?.is_active!==false?'selected':''}>Live</option><option value="false" ${p?.is_active===false?'selected':''}>Hidden</option></select><button class="btn gold" type="submit">Save design</button></form>`);
}
async function saveProduct(e,id){e.preventDefault();let f=new FormData(e.target),size_prices={};['XS','S','M','L','XL','XXL'].forEach(s=>{let v=String(f.get('size_'+s)||'').trim();if(v!=='')size_prices[s]=Number(v)});let row={name:f.get('name'),price:Number(f.get('price')),category:f.get('category'),description:f.get('description'),image_url:f.get('image_url')||fallbackImg,is_active:f.get('active')==='true',size_prices,updated_at:new Date().toISOString()};let r=id?await db.from('products').update(row).eq('id',id):await db.from('products').insert(row);if(r.error)return toast(r.error.message);await loadProducts();adminHome();toast('Catalog updated')}
...
document.addEventListener('pointerdown',e=>{
  ...
  const s=document.createElement('span');s.className='touch-ripple';s.style.left=x+'px';s.style.top=y+'px';b.appendChild(s);setTimeout(()=>s.remove(),700);
},{passive:true});
...
window.openAuth=openAuth;window.openMyOrders=openMyOrders;window.openProduct=openProduct;window.openCart=openCart;window.addToCart=addToCart;window.changeQty=changeQty;window.checkout=checkout;window.placeOrder=placeOrder;window.sendEmailOtp=sendEmailOtp;window.verifyEmailOtp=verifyEmailOtp;window.signOut=signOut;window.openChat=openChat;window.sendCustomerMessage=sendCustomerMessage;window.openAdmin=openAdmin;window.adminHome=adminHome;window.adminProducts=adminProducts;window.adminOrders=adminOrders;window.adminChats=adminChats;window.adminLoginActivity=adminLoginActivity;window.editProduct=editProduct;window.saveProduct=saveProduct;window.updateOrder=updateOrder;window.markPaid=markPaid;window.adminOpenChat=adminOpenChat;window.sendAdminMessage=sendAdminMessage;window.closeModal=closeModal;
