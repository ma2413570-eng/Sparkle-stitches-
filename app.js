...
async function uploadImage(bucket,file,pathPrefix){
  ...
  const {error}=await db.storage.from(bucket).upload(path,file,{upsert:false,contentType:file.type,cacheControl:'3600'});
...
async function openMyOrders(){
  ...
  const cards=(orders||[]).map(o=>`<article class="order-card glass"><div class="row"><div><div class="eyebrow">Order</div><b>#${esc(o.id.slice(0,8).toUpperCase())}</b></div><span class="order-badge ${orderStatusClass(o.order_status)}">${esc(orderStatusLabel(o.order_status))}</span></div><div class="muted" style="margin-top:6px">${new Date(o.created_at).toLocaleString('en-IN')}</div><div class="order-items">${items.filter(i=>i.order_id===o.id).map(i=>`<div class="order-item"><span>${esc(i.product_name)} × ${i.quantity}</span><b>${money(Number(i.unit_price)*Number(i.quantity))}</b></div>`).join('')}</div>${orderSteps(o.order_status)}<div class="order-summary"><span>Payment: <b>${esc(o.payment_status||'PENDING')}</b></span><strong>${money(o.total_amount)}</strong></div></article>`).join('');
  modal(`<div class="modal-head"><div><div class="eyebrow">Your account</div><h3>My Orders</h3></div><button class="btn" onclick="closeModal()">×</button></div><div class="my-orders">${cards||'<div class="empty">You have not placed any orders yet.</div>'}</div>`);
  subscribeCustomerOrderUpdates();
}
async function openNotifications(){
  if(!user){openAuth();return}
  const {data,error}=await db.from('notifications').select('*').eq('user_id',user.id).order('created_at',{ascending:false}).limit(50);
  if(error)return toast(error.message);
  modal(`<div class="modal-head"><div><div class="eyebrow">Updates</div><h3>Notifications</h3></div><button class="btn" onclick="closeModal()">×</button></div><div class="notifications-list">${(data||[]).map(n=>`<div class="notification ${n.read_at?'read':'unread'}"><b>${esc(n.title)}</b><div>${esc(n.message)}</div><small class="muted">${new Date(n.created_at).toLocaleString('en-IN')}</small></div>`).join('')||'<div class="empty">No notifications yet.</div>'}</div>`);
...
async function submitCustomOrder(e){
  ...
  try{
    toast('Uploading your design…');
    const urls=[]; for(const f of files) urls.push(await uploadImage(CUSTOM_BUCKET,f,user.id));
    const r=await db.from('custom_orders').insert({user_id:user.id,message:String(new FormData(e.target).get('message')||'').trim(),photo_urls:urls}).select().single();
    if(r.error)throw r.error;
    closeModal();browserNotify('Custom order sent','We received your design request.');
  }catch(err){toast(err.message||'Could not send custom order')}
}
async function subscribeCustomerOrderUpdates(){
  if(!user)return;
  clearRealtime();
  const ch=db.channel('customer-live-'+user.id).on('postgres_changes',{event:'UPDATE',schema:'public',table:'orders',filter:`user_id=eq.${user.id}`},payload=>{browserNotify('Order update',`Your order is now ${orderStatusLabel(payload.new.order_status)}.`); if(document.querySelector('.my-orders'))openMyOrders()}).on('postgres_changes',{event:'INSERT',schema:'public',table:'notifications',filter:`user_id=eq.${user.id}`},payload=>{browserNotify(payload.new.title,payload.new.message)}).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},payload=>{if(currentConv&&payload.new.conversation_id===currentConv.id&&payload.new.sender_id!==user.id)showCustomerChat()}).subscribe();
  realtimeChannels.push(ch);
}
async function subscribeAdminLive(){
  clearRealtime();
  const ch=db.channel('admin-live').on('postgres_changes',{event:'INSERT',schema:'public',table:'notifications'},payload=>{if(payload.new.admin_only)browserNotify(payload.new.title,payload.new.message);if(isAdmin&&document.querySelector('#adminContent'))adminProducts()}).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},payload=>{if(currentConv&&payload.new.conversation_id===currentConv.id&&payload.new.sender_id!==user.id)adminOpenChat(currentConv.id)}).subscribe();
  realtimeChannels.push(ch);
}

function openAuth(){if(user){modal(`<div class="modal-head"><div><div class="eyebrow">Welcome back</div><h3>${esc(user.email)}</h3></div><button class="btn" onclick="closeModal()">×</button></div><div class="form"><button class="btn gold" style="width:100%" onclick="openMyOrders()">📦 My Orders</button><button class="btn" style="width:100%" onclick="openNotifications()">🔔 Notifications <span id="accountNotifCount"></span></button><button class="btn" style="width:100%" onclick="openCustomOrder()">✨ Custom order</button><button class="btn" style="width:100%" onclick="signOut()">Sign out</button></div>`);return}modal(`<div class="modal-head"><div><div class="eyebrow">Secure access</div><h3>Sign in</h3></div><button class="btn" onclick="closeModal()">×</button></div><p class="muted">Use a six-digit email OTP. No password needed.</p><div class="form"><input id="email" class="input" type="email" placeholder="you@example.com"><button class="btn gold" onclick="sendEmailOtp()">Send email OTP</button><div class="notice">Phone OTP is designed into the app, but SMS delivery needs an SMS provider. Since we're keeping your launch cost at ₹0, email OTP is enabled first.</div></div>`)}
...
async function showCustomerChat(){
  let {data:msgs}=await db.from('messages').select('*').eq('conversation_id',currentConv.id).order('created_at');
  modal(`<div class="modal-head"><div><div class="eyebrow">Sparkle Stitches</div><h3>Support chat</h3></div><button class="btn" onclick="closeModal()">×</button></div><div id="customerMessages" class="messages glass" style="border-radius:18px">${(msgs||[]).map(m=>`<div class="bubble ${m.sender_id===user.id?'me':'them'}">${esc(m.message)}</div>`).join('')||'<div class="empty">Ask us anything about designs, orders or custom work.</div>'}</div><div class="chat-send"><input id="customerMsg" class="input" placeholder="Type your message…" onkeydown="if(event.key==='Enter')sendCustomerMessage()"><button class="btn gold" onclick="sendCustomerMessage()">Send</button></div><button class="btn" style="width:100%;margin-top:10px" onclick="openCustomOrder()">📷 Send custom design (up to 3 photos)</button>`);
  subscribeCustomerChat();
}
async function subscribeCustomerChat(){
  if(!currentConv)return;
  realtimeChannels.forEach(ch=>{if(ch.topic?.includes('customer-chat-')){try{db.removeChannel(ch)}catch(e){}}});
  const ch=db.channel('customer-chat-'+currentConv.id).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:`conversation_id=eq.${currentConv.id}`},payload=>{if(payload.new.sender_id!==user.id)showCustomerChat()}).subscribe();
  realtimeChannels.push(ch);
}
...
async function editProduct(id){
  ...
  modal(`<div class="modal-head"><div><div class="eyebrow">Catalog</div><h3>${p?'Edit design':'New design'}</h3></div><button class="btn" onclick="adminHome()">×</button></div><form class="form" id="productForm"><input class="input" name="name" placeholder="Product name" value="${esc(p?.name||'')}" required><div class="two"><input class="input" name="price" type="number" min="0" step="1" placeholder="Price" value="${p?.price||''}" required><input class="input" name="category" placeholder="Category" value="${esc(p?.category||'Blouse Ari Work')}" required></div><textarea class="textarea" name="description" rows="3" placeholder="Description">${esc(p?.description||'')}</textarea><label class="muted">Product photo</label><input class="input" name="image_file" type="file" accept="image/*"><div class="muted">Upload a photo from your phone. Max 8 MB.</div><input class="input" name="image_url" placeholder="Or paste image URL" value="${esc(p?.image_url||'')}"><select class="select" name="active"><option value="true" ${p?.is_active!==false?'selected':''}>Live</option><option value="false" ${p?.is_active===false?'selected':''}>Hidden</option></select><button class="btn gold" type="submit">Save design</button>${p?`<button class="btn danger" type="button" onclick="deleteProduct('${p.id}')">Delete this design</button>`:''}</form>`);
  $('#productForm').onsubmit=e=>saveProduct(e,id);
}
async function saveProduct(e,id){
  e.preventDefault(); let f=new FormData(e.target); let old=id?products.find(x=>x.id===id):null;
  let imageUrl=(String(f.get('image_url')||'').trim())||old?.image_url||fallbackImg; const file=f.get('image_file');
  try{if(file&&file.size){toast('Uploading photo…');imageUrl=await uploadImage(PRODUCT_BUCKET,file,'products')}}catch(err){return toast(err.message||'Photo upload failed')}
  ...
  if(id&&file&&old?.image_url){const path=storagePath(old.image_url,PRODUCT_BUCKET);if(path)await db.storage.from(PRODUCT_BUCKET).remove([path])}
...
async function deleteProduct(id){
  ...
  await loadProducts();adminHome();toast('Product deleted');
}

async function adminNotifications(){
  const {data,error}=await db.from('notifications').select('*').eq('admin_only',true).order('created_at',{ascending:false}).limit(50);
  if(error)return toast(error.message);
  $('#adminContent').innerHTML=`<div class="notifications-list">${(data||[]).map(n=>`<div class="notification ${n.read_at?'read':'unread'}"><b>${esc(n.title)}</b><div>${esc(n.message)}</div><small class="muted">${new Date(n.created_at).toLocaleString('en-IN')}</small></div>`).join('')||'<div class="empty">No admin notifications.</div>'}</div>`;
  await db.from('notifications').update({read_at:new Date().toISOString()}).eq('admin_only',true).is('read_at',null);
}
async function adminAdmins(){
  const r=await db.from('admin_users').select('email,created_at').order('created_at',{ascending:true});
  if(r.error)return toast(r.error.message);
  $('#adminContent').innerHTML=`<div class="eyebrow">Access control</div><h3>Admin accounts</h3><p class="muted">Add another email as an admin. That person can then sign in with the normal email OTP.</p><form class="form" id="addAdminForm"><input class="input" type="email" name="email" placeholder="newadmin@example.com" required><button class="btn gold" type="submit">Add admin</button></form><div class="table-wrap glass"><table class="table"><tr><th>Email</th><th>Added</th></tr>${(r.data||[]).map(a=>`<tr><td>${esc(a.email)}</td><td>${new Date(a.created_at).toLocaleString('en-IN')}</td></tr>`).join('')}</table></div>`;
  $('#addAdminForm').onsubmit=async e=>{e.preventDefault();const email=new FormData(e.target).get('email');const x=await db.rpc('grant_admin_by_email',{target_email:email});if(x.error)return toast(x.error.message);toast('Admin added successfully');adminAdmins()};
}
